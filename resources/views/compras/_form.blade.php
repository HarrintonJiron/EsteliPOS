    ...
    >
        ...
        <section class="flex min-h-0 flex-1 flex-col">
            <div class="border-b border-slate-200 bg-white px-4 py-3 sm:px-6">
                ...
                <p id="searchHint" class="mt-2 text-xs text-slate-500">Selecciona un proveedor para buscar cualquier producto activo.</p>
            </div>
            ...
            <div class="flex-1 overflow-y-auto px-4 py-4 sm:px-6">
                ...
                <div id="purchaseEmpty" class="mx-auto flex max-w-md flex-col items-center justify-center py-20 text-center">
                    ...
                    <p class="mt-1 text-sm text-slate-500">Selecciona un proveedor y agrega cualquier producto del inventario.</p>
                </div>
            </div>
        </section>
    </form>
...
(() => {
    ...
    function updateSearchState() {
        ...
        searchHint.textContent = hasSupplier
            ? 'Puedes agregar cualquier producto activo a la compra.'
            : 'Selecciona un proveedor para buscar cualquier producto activo.';
    ...
    function renderResults(products) {
        ...
        if (!products.length) {
            ...
            searchResults.innerHTML = `
                <div class="px-4 py-3 text-sm text-slate-500">No hay productos activos que coincidan.</div>
        ...
        searchResults.innerHTML = products.map((product, index) => `
            ...
            >
                <div class="min-w-0">
                    ...
                    <p class="text-xs text-slate-500">${escapeHtml(product.code)}${product.has_supplier_price ? ' · costo de este proveedor' : ' · producto disponible'}</p>
                </div>
    ...
    async function runSearch(term) {
        ...
        if (requestId !== searchRequestId || searchInput.value.trim() !== term) {
            return;
        }
    ...
    searchInput.addEventListener('focus', () => {
        ...
        if (supplierSelect.value && term !== '' && searchResults.classList.contains('hidden')) {
            runSearch(term);
        }
    });
