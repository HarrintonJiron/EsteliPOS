                    ...
                    @foreach($supplier->purchases as $p)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-2 text-center">
                                <span class="px-2 py-1 rounded-full text-xs {{ $p->status === 'completed' ? 'bg-green-100 text-green-700' : ($p->status === 'pending' ? 'bg-yellow-100 text-yellow-700' : ($p->status === 'ordered' ? 'bg-blue-100 text-blue-700' : 'bg-red-100 text-red-700')) }}">
                                    {{ $p->status === 'completed' ? 'Pagado' : ($p->status === 'pending' ? 'Por pagar' : ($p->status === 'ordered' ? 'Pedido en proceso' : 'Anulada')) }}
                                </span>
                            </td>
                            <td class="px-4 py-2 text-center">
