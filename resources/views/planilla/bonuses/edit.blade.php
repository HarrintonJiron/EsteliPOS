        ...
        <form action="{{ route('bonuses.update', $bonus) }}" method="POST">
            ...
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div class="col-span-2">
                    ...
                    <input type="text" disabled value="{{ $bonus->employee->name }} - {{ $bonus->employee->position }}"
                ...
                <div>
                    ...
                    <select name="type" required
                        ...
                        <option value="performance" @selected(old('type', $bonus->type) === 'performance')>Desempeño</option>
                        <option value="sales" @selected(old('type', $bonus->type) === 'sales')>Ventas</option>
                        <option value="attendance" @selected(old('type', $bonus->type) === 'attendance')>Asistencia</option>
                        <option value="christmas" @selected(old('type', $bonus->type) === 'christmas')>Navideño</option>
                        <option value="productivity" @selected(old('type', $bonus->type) === 'productivity')>Productividad</option>
                        <option value="other" @selected(old('type', $bonus->type) === 'other')>Otro</option>
                    </select>
                    @error('type')
                ...
                <div>
                    ...
                    <input type="number" name="amount" step="0.01" min="0" required value="{{ old('amount', $bonus->amount) }}"
                ...
                <div>
                    ...
                    <input type="date" name="date" required value="{{ old('date', $bonus->date->format('Y-m-d')) }}"
                ...
                <div class="col-span-2">
                    ...
                    <textarea name="reason" rows="3"
                        class="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">{{ old('reason', $bonus->reason) }}</textarea>
                </div>
            </div>
