...
<div class="space-y-6">
    ...
    <div class="flex flex-col gap-3 lg:flex-row lg:justify-between lg:items-center">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('envios.create', ['sale_id' => $sale->id]) }}" class="bg-sky-600 text-white px-4 py-2 rounded-lg hover:bg-sky-700 flex items-center gap-2">🚚 Preparar envío</a>
            @endif
            ...
            @endif
            <a href="{{ route('facturacion.pdf', ['sale_id' => $sale->id]) }}" target="_blank" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 flex items-center gap-2">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"></path></svg>
                PDF
            </a>
            <a href="{{ route('facturacion.print', ['sale_id' => $sale->id]) }}" target="_blank" class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center gap-2">
            ...
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y">
                    @foreach($sale->details as $detail)
                        <tr>
                            <td class="px-6 py-4">{{ $detail->product->name ?? 'N/A' }} <span class="text-xs text-slate-500">({{ $detail->unit?->abbreviation ?? $detail->product?->baseUnitLabel() ?? 'und' }})</span></td>
                            <td class="px-6 py-4">{{ $detail->quantity }} {{ $detail->unit?->abbreviation ?? $detail->product?->baseUnitLabel() ?? 'und' }}</td>
