...
Route::middleware(['auth'])->group(function () {
    ...
    Route::middleware(['module:configuracion', 'permission:configuracion.view'])->prefix('settings')->name('settings.')->group(function () {
        ...
        Route::middleware('permission:configuracion.edit')->group(function () {
            ...
            Route::get('/taxes/{tax}/edit', [\App\Http\Controllers\TaxController::class, 'edit'])->name('taxes.edit');
            Route::match(['put', 'patch'], '/taxes/{tax}', [\App\Http\Controllers\TaxController::class, 'update'])->name('taxes.update');
    ...
    Route::middleware(['module:contabilidad', 'permission:contabilidad.view'])->prefix('contabilidad')->name('contabilidad.')->group(function () {
        Route::get('/', \App\Http\Controllers\AccountingDashboardController::class)->name('dashboard');
        Route::get('/reportes/{report}/pdf', [\App\Http\Controllers\AccountingReportExportController::class, 'pdf'])
            ->whereIn('report', ['estado-resultados', 'balance-general', 'flujo-caja', 'balance-comprobacion', 'diario-general', 'mayor-general'])
            ->name('reportes.pdf');
        Route::get('/reportes/{report}/excel', [\App\Http\Controllers\AccountingReportExportController::class, 'excel'])
            ->whereIn('report', ['estado-resultados', 'balance-general', 'flujo-caja', 'balance-comprobacion', 'diario-general', 'mayor-general'])
            ->name('reportes.excel');
        Route::get('/cuentas', [\App\Http\Controllers\AccountController::class, 'index'])->name('cuentas.index');
        Route::middleware(['permission:contabilidad.create'])->group(function () {
            Route::get('/cuentas/create', [\App\Http\Controllers\AccountController::class, 'create'])->name('cuentas.create');
            Route::post('/cuentas', [\App\Http\Controllers\AccountController::class, 'store'])->name('cuentas.store');
        });
        Route::middleware(['permission:contabilidad.edit'])->group(function () {
            Route::get('/cuentas/{account}/edit', [\App\Http\Controllers\AccountController::class, 'edit'])->name('cuentas.edit');
            Route::match(['put', 'patch'], '/cuentas/{account}', [\App\Http\Controllers\AccountController::class, 'update'])->name('cuentas.update');
        });
        Route::delete('/cuentas/{account}', [\App\Http\Controllers\AccountController::class, 'destroy'])
            ->middleware(['permission:contabilidad.delete'])
            ->name('cuentas.destroy');

        // Asientos contables (pólizas)
        Route::get('/asientos', [\App\Http\Controllers\JournalEntryController::class, 'index'])->name('asientos.index');
        Route::get('/asientos/{journalEntry}', [\App\Http\Controllers\JournalEntryController::class, 'show'])->name('asientos.show');
        Route::middleware(['permission:contabilidad.create'])->group(function () {
            Route::get('/asientos-nuevo', [\App\Http\Controllers\JournalEntryController::class, 'create'])->name('asientos.create');
            Route::post('/asientos', [\App\Http\Controllers\JournalEntryController::class, 'store'])->name('asientos.store');
        });
        Route::middleware(['permission:contabilidad.edit'])->group(function () {
            Route::post('/asientos/{journalEntry}/contabilizar', [\App\Http\Controllers\JournalEntryController::class, 'post'])->name('asientos.post');
            Route::post('/asientos/{journalEntry}/anular', [\App\Http\Controllers\JournalEntryController::class, 'void'])->name('asientos.void');
        });
        Route::delete('/asientos/{journalEntry}', [\App\Http\Controllers\JournalEntryController::class, 'destroy'])
            ->middleware(['permission:contabilidad.delete'])
            ->name('asientos.destroy');

        // Diario General
        Route::get('/diario', [\App\Http\Controllers\DiarioController::class, 'index'])->name('diario.index');
        Route::get('/diario/export', [\App\Http\Controllers\DiarioController::class, 'export'])->name('diario.export');

        // Mayor General
        Route::get('/mayor', [\App\Http\Controllers\LedgerController::class, 'index'])->name('mayor.index');

        // Balance de Comprobación
        Route::get('/balance-comprobacion', [\App\Http\Controllers\TrialBalanceController::class, 'index'])->name('balance-comprobacion.index');
        Route::get('/balance-comprobacion/export', [\App\Http\Controllers\TrialBalanceController::class, 'export'])->name('balance-comprobacion.export');

        // Estado de Resultados
        Route::get('/estado-resultados', [\App\Http\Controllers\IncomeStatementController::class, 'index'])->name('estado-resultados.index');
        Route::get('/estado-resultados/export', [\App\Http\Controllers\IncomeStatementController::class, 'export'])->name('estado-resultados.export');

        // Balance General
        Route::get('/balance-general', [\App\Http\Controllers\BalanceSheetController::class, 'index'])->name('balance-general.index');
        Route::get('/balance-general/export', [\App\Http\Controllers\BalanceSheetController::class, 'export'])->name('balance-general.export');

        // Flujo de Caja
        Route::get('/flujo-caja', [\App\Http\Controllers\CashFlowController::class, 'index'])->name('flujo-caja.index');
        Route::get('/flujo-caja/export', [\App\Http\Controllers\CashFlowController::class, 'export'])->name('flujo-caja.export');

        // Centros de Costo
        Route::get('/centros-costo', [\App\Http\Controllers\CostCenterController::class, 'index'])->name('centros-costo.index');
        Route::middleware(['permission:contabilidad.create'])->group(function () {
            Route::get('/centros-costo/create', [\App\Http\Controllers\CostCenterController::class, 'create'])->name('centros-costo.create');
            Route::post('/centros-costo', [\App\Http\Controllers\CostCenterController::class, 'store'])->name('centros-costo.store');
        });
        Route::middleware(['permission:contabilidad.edit'])->group(function () {
            Route::get('/centros-costo/{centro_costo}/edit', [\App\Http\Controllers\CostCenterController::class, 'edit'])->name('centros-costo.edit');
            Route::match(['put', 'patch'], '/centros-costo/{centro_costo}', [\App\Http\Controllers\CostCenterController::class, 'update'])->name('centros-costo.update');
        });
        Route::delete('/centros-costo/{centro_costo}', [\App\Http\Controllers\CostCenterController::class, 'destroy'])
            ->middleware(['permission:contabilidad.delete'])
            ->name('centros-costo.destroy');

        // Períodos Fiscales (cierre mensual/anual)
        Route::get('/periodos', [\App\Http\Controllers\FiscalPeriodController::class, 'index'])->name('periodos.index');
        Route::middleware(['permission:contabilidad.close_period'])->group(function () {
            Route::post('/periodos/{periodo}/cerrar', [\App\Http\Controllers\FiscalPeriodController::class, 'closeMonth'])->name('periodos.cerrar');
            Route::post('/periodos/{periodo}/reabrir', [\App\Http\Controllers\FiscalPeriodController::class, 'reopenMonth'])->name('periodos.reabrir');
            Route::post('/periodos/{periodo}/cerrar-anio', [\App\Http\Controllers\FiscalPeriodController::class, 'closeYear'])->name('periodos.cerrar-anio');
            Route::post('/periodos/{periodo}/reabrir-anio', [\App\Http\Controllers\FiscalPeriodController::class, 'reopenYear'])->name('periodos.reabrir-anio');
        });
